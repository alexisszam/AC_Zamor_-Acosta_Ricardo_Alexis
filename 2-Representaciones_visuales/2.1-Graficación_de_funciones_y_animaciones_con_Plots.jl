...
# ╔═╡ 8b1dd308-0e0b-47bb-83e8-ba64da2c4d46
@bind d Slider(0.1:0.1:2, default=0.1)

# ╔═╡ 9fc79ca8-4100-4cab-9f3f-a55f886c700b
d

# ╔═╡ 6518d3cc-cc2f-40f5-b90c-5e24b1a19369
begin
	plot(cos,-π:d:π, color = "red", label = L"\cos(x)", marker = true)
	plot!(sin,-π:d:π, title = "Funciones Trigonométricas", xlabel = "x", ylabel = "y", color = "purple", label = L"\sin(x)", marker = true)
end
...
# ╔═╡ 0e1323bc-58c6-49f7-9270-073a6e7081e8
begin
...
begin
	a = 61.8
	r = 10
	θ = 76               #Definimos todas las variables que usaremos
	t = 2.19
	Tp(x)=1.8+(9.7(-x)/2.42)-(0.5*(9.8*(((-x)/2.42)^2)))+60    #Fórmula que usaremos para graficar el Tiro parabólico

	@gif for t in 0
	scatter(Tp.( range(0,20, step = 0.1) .-t),
		 legend = false, title = "Tiro parabólico", xlims=(0,67), ylims=(0,63),shape=(:circle), markercolor=(:red), markersize= 5)
...
# ╔═╡ 5623bc54-e8fc-4dc5-aae3-a68f9f17e256
@bind a1 Slider(0:1:100, default=0)

# ╔═╡ f146e00e-8812-464c-bc74-3c5dcba3857d
# Tu código (comentado) va aquí :D
	a1

# ╔═╡ 1551aa8f-9fc9-4e53-a5a5-dd51389ba55b
@bind r1 Slider(0:1:1000, default=100)

# ╔═╡ a996b404-4573-4e9b-bf3b-64ef4d0fb300
r1

# ╔═╡ 9f637616-58b8-46cb-9b32-a1759bb3b812
@bind θ1 Slider(0:π/2:2π, default=π/3)

# ╔═╡ d28e5880-42bf-4f6d-ab78-2d2cdca15403
θ1

# ╔═╡ e01df960-7f34-453c-9c6a-0c1cdf5f9ea4
t1 = (0:0.1:10)

# ╔═╡ e94f9f27-535a-4fdb-812d-071199c4d92e


# ╔═╡ 803caf56-67ca-432d-a79e-4d87782f518b
x1 = (r1*t1*cos(θ1))

# ╔═╡ 29483c9e-e1d3-466f-a1e5-bff7123502d0
t2 = t1*t1

# ╔═╡ 58e77f54-c558-49e1-b293-ce92289b9b37
y1 = (a1.+(r1*t1*sin(θ1))-(9.8*t2)/2)

# ╔═╡ e1ab44e3-68e3-45d0-a1f0-3f6f93303873
plot(x1,y1)

# ╔═╡ 91030a7f-efef-4f17-9f45-6b69516ce90c
...
# ╔═╡ 00080de1-ae8f-4770-a189-af57c8e66b28
begin
	x = 1
	h(y) = cos(x)+sin(y) # Definimos la función f
	
	@gif for t in 0:0.1:2π # para t en 0:0.1:2π
	plot(h.( range(0,2π, step = 0.1).-t),
...
# ╔═╡ 13c2594e-d702-4a61-83ef-b851f618a525
t3 = 5

# ╔═╡ d482e513-10f7-4c78-9f93-05bdec5b13ad
x2=(0:0.1:2π)

# ╔═╡ 19a48f63-ccc9-49bb-bb82-9d0c71865216
y2=(0:0.1:2π*t3)

# ╔═╡ 885d30f5-d3b1-4527-b038-91821e802f80
begin
ym= 2π*t3
	@gif for t3 in (0:0.1:3)
	surface(x2,y2,h,yaxis=(0,ym),zaxis=(-3,3), background= "black", color=(:redsblues))
	end
end
