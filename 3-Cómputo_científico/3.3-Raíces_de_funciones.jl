...
# ╔═╡ e51b7fb3-9199-4234-9c23-20faa9d1c84e
using Plots, PlutoUI
...
# ╔═╡ b80260e8-2b1c-4820-88d8-143e3dc072c3
begin                            #Definimos nuestros argumentos para la función
	f(x)= 3x^3 + 6               
	f′(x)= 9x^2
	x0= 2
	i=1
...
# ╔═╡ f2bc2a8a-6f35-4c70-89b7-4e98ee5eab1d
# Tu código (comentado) va aquí :D
function newton0(f, f′, x0, n)    #Definimos el nombre de la función y sus parámetros
	x1 = 0                       #Valor inicial cero
	for i in 1:(i+1)             #Para i+1 iteraciones
		x1 = x0 - f(x0)/f′(x0)   #Operación de método de Newton
		x0 = x1                  #Nuestro nuevo valor x0 es el valor obtenido de la operación del paso anterior
	end                          #El proceso se repite hasta llegar a i+1
		return x1                #Cuando termine devuelve el último valor de x1 obtenido
end
...
# ╔═╡ 981ee1ed-0672-40be-8d54-043a8947a649
# Tu código (comentado) va aquí :D
function newton(f, f′, x0, n)    #Definimos el nombre de la función y sus parámetros
	x1 = 0                       #Valor inicial cero
	for i in 1:n                 #Para n iteraciones
		x1 = x0 - f(x0)/f′(x0)   #Operación de método de Newton
		x0 = x1                  #Nuestro nuevo valor x0 es el valor obtenido de la operación del paso anterior
	end                          #El proceso se repite hasta llegar a n
		return x1                #Cuando termine devuelve el último valor de x1 obtenido
end

# ╔═╡ 6656518b-48fb-431a-852c-2dea4bbc5422
n=100                           #Definimos el valor de n

# ╔═╡ d9491619-8a01-476d-8b73-3d32769ab19a
newton0(f, f′, x0, n)

# ╔═╡ 483e176e-ce6b-432d-9403-7f830ea766a6
raíz1=newton(f, f′, x0, n)       #El valor de nuestra raíz será el resultado de aplicar la función newton con los parámetros f,f′,x0,n dados

# ╔═╡ 4201e94c-a8cf-4cf0-86c7-f1834cd6b814
raíz1                            #Valor de la raíz encontrada

# ╔═╡ 82103bde-6482-4387-96e0-060411266d49
f(raíz1)                         #Comprobación que nuestra raíz es correcta
...
# Tu código (comentado) va aquí :D
function newtonDistancia(f, f′, x0, 𝝐)       #Definimos el nombre de nuestra función y sus parámetros
	x1= 0                                    #Valor inicial
	error= 𝝐 + 1                             #Al valor que definimos para 𝝐 (que será la presición que querramos) añade una unidad
	while error> 𝝐                           #Mientras que el error sea mayor que 𝝐 entonces
		x1= x0 -f(x0)/f′(x0)                 #Fórmula método de Newton
		error= abs(x0 - x1)                  #Redefinimos al valor del error como la distancia entre x y x1
		x0= x1                               #Redifinimos el valor de x0 con lo obtenido con el método de Newton
	end
	return x1                                #Regresa el valor de x1 cuando el error sea menor o igual a 𝝐
end

# ╔═╡ 13047da7-befb-4ac2-92f5-37a45e50bb0e
𝝐= 0.0001                                    #Definimos el valor de 𝝐

# ╔═╡ 77f20ee4-d9f7-435f-8fa2-a13c9ec0102c
raíz2= newtonDistancia(f, f′, x0, 𝝐)         #El valor de nuestra raíz será el resultado de aplicar la función newtonDistancia con los parámetros f,f′,x0,𝝐 dados

# ╔═╡ 282c96d1-6257-42e7-a251-90e5ae814f45
raíz2                                        #Valor de la raíz encontrada

# ╔═╡ 67d340f9-5351-4a10-b551-a92f837e6de0
f(raíz2)                                     #Comprobación que nuestra raíz es correcta
...
# ╔═╡ 4a31a48c-011d-405f-b257-d892659e8657
# Tu código (comentado) va aquí :D
function derivadaNumérica(f,x,h)                   #Definimos el nombre de nuestra función y sus parámetros
     h * (f(x+(1/h))-f(x))                         #Definimos  nuestra hsiendo este un valor grande
end                                                #Devuelve el valor de la derivada aplicando la fórmula

# ╔═╡ 021313ee-bcc2-4831-934c-6823f331f5cf
h= 10000

# ╔═╡ f9a23775-2c17-4125-a121-ef460b8a2fa1
derivadaNumérica(f,1,h)

# ╔═╡ 2f5e5c94-3c74-4c02-a0ed-b1804e613745
# Tu código (comentado) va aquí :D                  Otro método de derivadaNumérica 
function derivadaNuméricaB(fun,x)                  #Definimos el nombre de nuestra función y sus parámetros
     h = (eps()^(1/3))*max(abs(x), 1.0)            #Definimos  uestra hsiendo este un valor grande 
    return (fun(x+h)-fun(x-h))/(2*h)               #Devuelve el valor de la derivada aplicando la fórmula
end

# ╔═╡ 60d31434-c24e-4259-82d6-6a2ff6621971
derivadaNuméricaB(f,1)                              #Resultada de nuestra defivada(9x^2) de la función f evaluado en 1
...
# Tu código (comentado) va aquí :D
function newtonDN(f,x0,n)                             #Definimos el nombre de nuestra función y sus parámetros
	x1= 0                                             #Valor inicial
	for i in 1:n                                      #Para n iteraciones
		x1= x0 - f(x0)/derivadaNumérica(f,x0,h)       #Operación para la derivada implementando el método de Newton
		x0= x1                                        #Redifinimos el valor de x0 con lo obtenido con el paso anterior
	end                                               
	return x1                                         #Devuelve el último valor obtenido de x1
end

# ╔═╡ a049154d-86f4-4a2e-b0da-c6561604dfea
raíz3= newtonDN(f,x0,n)                               #El valor de nuestra raíz será el resultado de aplicar la función newtonDN con los parámetros f,x0,n dados

# ╔═╡ 4cb41954-c38a-40ff-a102-2af1474d46e9
raíz3                                                  #Valor de la raíz encontrada y vemos que es el mismo del ejercicio anterior

# ╔═╡ 6240864f-7d8e-487f-a44e-066d98a9c112
f(raíz3)                                               #Comprobación que nuestra raíz es correcta
...
# ╔═╡ 9910fc4f-5438-48fa-8784-c7149ad8629d
@bind x00 Slider(-10:0.1:10, default=0)

# ╔═╡ 21ea8174-b5c2-4098-b399-3cff9076c076
x00

# ╔═╡ 4c46377f-4b4f-4684-8dd2-048a3b23ee4d
@bind h0 Slider(1000:1000:1000000, default=500000)

# ╔═╡ e9aca9bd-82c5-4890-a482-c170e09397e9
h0

# ╔═╡ 84b9e44a-14b5-4724-9acc-d38ea0a2cda2
@bind n0 Slider(1:1000, default=500)

# ╔═╡ 6564afc1-ab9b-49ab-aa80-d46c5ae7a71a
n0

# ╔═╡ 005372a4-35cc-4d81-846e-0aa9b66a6ce5
 # Luego, usa esta celda para reasignar `f`
 f1(x)= x^2 -2
...
# Tu código (comentado) va aquí :D
function newtonDN0(f1,x0,n)                            #Definimos el nombre de nuestra función y sus parámetros
	x1= 0                                             #Valor inicial
	for i in 1:n                                      #Para n iteraciones
		x1= x0 - f(x0)/derivadaNumérica(f,x0,h)       #Operación para la derivada implementando el método de Newton
		x0= x1                                        #Redifinimos el valor de x0 con lo obtenido con el paso anterior
...
# ╔═╡ ae4a58fb-e53c-403a-90b3-fbc95ce197d8
newtonDN0(f1,x00,n0)

# ╔═╡ 5c199f19-1e17-492c-9236-fae2084cf8be
plot(f1,-3:3)
